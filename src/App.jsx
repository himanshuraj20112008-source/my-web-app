...
const RISK_ENGINE = {
  ...
  phone(raw) {
    ...
    const is10=len===10;
    if (!is10&&!isE164) { indicators.push(`Invalid length (${len} digits) — expected 10 or 12 with country code`); weights.push(55); structuralFlag="❌ Invalid Length"; }
    const num=isE164?cleaned.slice(2):cleaned;
    if (cleaned.length===12&&!cleaned.startsWith("91")) { indicators.push(`Invalid/unrecognized country code (+${cleaned.slice(0,2)})`); weights.push(40); structuralFlag=structuralFlag||"❌ Invalid Country Code"; }
    if (/^(.)\1{9}$/.test(num)) { indicators.push("All digits identical — not a real phone number"); weights.push(95); structuralFlag="🔴 Invalid Number"; }
    else if (/^(0123456789|1234567890)$/.test(num)) { indicators.push("Sequential digits — synthetic number"); weights.push(75); structuralFlag="🟠 Sequential Digits"; }
    else if (/^(9876543210)$/.test(num)) { indicators.push("Descending sequential digits — synthetic number"); weights.push(75); structuralFlag="🟠 Descending Sequence"; }
    else if (/(.)\1{4,}/.test(num)) { const match=num.match(/(.)\1{4,}/); const rep=match[1]; const repCount=(num.split(rep).length-1); if(repCount>=5){indicators.push(`Digit '${rep}' repeats ${repCount}x — excessive repetition`);weights.push(55);structuralFlag="🟡 Excessive Repetition";} }
    const entropy=calcEntropy(num);
    if (entropy<1.8&&!indicators.length) { indicators.push(`Very low digit entropy (${entropy.toFixed(2)}) — low randomness`); weights.push(28); }
    if (is10&&/^[0-5]/.test(num)) { indicators.push(`Number starts with '${num[0]}' — Indian mobile numbers start with 6-9`); weights.push(50); structuralFlag=structuralFlag||"🔴 Invalid Number"; }
    if (DB.phoneBlacklist.has(num)||DB.phoneBlacklist.has(cleaned)) { indicators.push("🚨 Matched fraud blacklist database"); weights.push(90); structuralFlag=structuralFlag||"🔴 Blacklisted Number"; }
    const communityData=DB.communityReports[num]||DB.communityReports[cleaned];
    if (communityData) { indicators.push(`Community reported ${communityData.reports}x — category: ${communityData.category}`); weights.push(Math.min(communityData.reports*4,45)); }
    const score=Math.min(weights.reduce((a,b)=>a+b,0),100);
    const level=score>=75?"critical":score>=50?"high":score>=25?"medium":"low";
    return { score, level, structuralFlag, indicators:indicators.length?indicators:["No suspicious patterns detected — number appears structurally valid"], recommendation:score>=75?"Do NOT answer or call back. Block this number immediately and report to TRAI DND (1909).":score>=50?"Exercise high caution. Do not share OTP, PIN, Aadhaar, or any personal details.":score>=25?"Some pattern anomalies found. Verify caller identity independently before sharing any info.":"Number appears structurally valid. Always verify caller identity before sharing sensitive data." };
  },
          ...
          {sections.map((line, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                lineHeight: 1.7,
                padding: isHeader ? "6px 0 2px" : "0",
                borderTop: isHeader && i > 0 ? "1px solid rgba(255,255,255,0.05)" : "none",
                paddingTop: isHeader && i > 0 ? 10 : undefined,
              }}>
                {cleanLine}
              </div>
            );
    ...
    <div className="fu" style={{padding:"22px 18px"}}>
      ...
      <div className="glass" style={{padding:18,marginBottom:14}}>
        ...
        <button className="btn-prime" onClick={runScan} disabled={loading||!input.trim()} style={{marginTop:11,padding:"11px 26px",fontSize:13}}>
          {loading?(tab==="url"?"⏳ Checking Safe Browsing…":"⏳ Analyzing…"):"🔬 Run Analysis"}
        </button>
      </div>
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg,maxWidth:760,margin:"0 auto"}}>
        ...
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:760,background:"rgba(8,14,28,0.97)",borderTop:`1px solid ${C.border}`,display:"flex",zIndex:100}}>
          {NAV.map(n=>(
            <button key={n.id} className="tab" onClick={()=>setPage(n.id)} style={{flex:1,padding:"10px 4px 9px",fontSize:9,color:page===n.id?C.cyan:C.muted,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
