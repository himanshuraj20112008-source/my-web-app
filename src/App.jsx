     ...
     <div className="mobile-tabs" style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
      ...
      <div className="glass" style={{padding:18,marginBottom:14}}>
        ...
        {cur.big?(
          ...
          <input className="ifield" type={cur.mask?"password":"text"} value={input} onChange={e=>setInput(e.target.value)} placeholder={cur.ph} onKeyDown={e=>e.key==="Enter"&&runScan()} style={{padding:"11px 13px",fontSize:13}}/>
        )}
...
function AssistantPage() {
  ...
  async function send(text) {
    ...
    const reply=await callAI({messages:history,system:"You are SentinelX AI, a helpful and knowledgeable assistant built into a cybersecurity app. Your specialty is cybersecurity — scams, phishing, UPI fraud, fake messages, and online safety. You can also answer general knowledge questions on any topic accurately and helpfully. Never refuse a question as 'out of scope'.\n\nRESPONSE LENGTH (very important): By default, keep answers SHORT — 3 to 6 sentences, covering only the most important point directly. Do NOT add extra sections, checklists, tables, or numbered step-by-step breakdowns unless the user explicitly asks for more detail, steps, or a full explanation (phrases like 'explain in detail', 'give me steps', 'how do I', 'tell me more'). A short answer is not one line — it's a few clear sentences with the key facts, not padded with disclaimers or repeated points.\n\nFORMATTING RULES (very important): Use **bold** only for 1-2 key terms, not entire headings. Do NOT use markdown headers (##), tables, or horizontal rules (---) — this app cannot render them and they will show as broken symbols. If listing steps, use plain numbered lines (1. 2. 3.) with no header above them. Keep paragraphs short. No em-dash-heavy or checklist-heavy answers by default.\n\nKeep cybersecurity answers practical; for general topics, answer normally. Remember conversation context."});
            ...
            <>
              ...
              <div className="fu" style={{position:"absolute",top:"calc(100% + 8px)",right:0,minWidth:190,padding:8,zIndex:200,display:"flex",flexDirection:"column",gap:4,background:"#0D1526",border:`1px solid ${C.border}`,borderRadius:16,boxShadow:"0 8px 32px rgba(0,0,0,0.5)"}}>
                <button onClick={()=>{setShowScamAlert(true);setShowMenu(false);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:8,background:"transparent",border:"none",color:C.warning,cursor:"pointer",fontFamily:"Inter,sans-serif",fontSize:13,fontWeight:600,textAlign:"left"}}>
                  <AlertTriangle size={15}/> Scam Alert
                  {!scamAlertLoading && scamAlertData && !scamAlertData.error && scamAlertData.title && (
        ...
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:760,background:"rgba(8,14,28,0.97)",borderTop:`1px solid ${C.border}`,display:"flex",zIndex:100,paddingBottom:"env(safe-area-inset-bottom)"}}>
          {NAV.map(n=>(
            <button key={n.id} className="tab mobile-nav-btn" onClick={()=>setPage(n.id)} style={{flex:1,padding:"10px 4px 9px",fontSize:9,color:page===n.id?C.cyan:C.muted,display:"flex",flexDirection:"column",alignItems:"center",gap:2,justifyContent:"center",minHeight:48}}>
              <n.icon size={18} strokeWidth={page===n.id?2.4:1.8}/>{n.id}
            </button>
          ))}
        </div>
                <button
